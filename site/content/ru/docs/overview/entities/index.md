...
«Неделимая последовательность» в определении операции означает:
1. шаги операции выполняются последовательно, без логических условий и циклов (условия и циклы нуждаются в состоянии, поэтому содержатся в сервисах);
1. операция не может обращаться к другим операциям, поскольку это прерывает последовательность шагов.

Эти свойства позволяют легко описывать операцию с помощью **диаграмм последовательности**. Задача операции — организовать _обмен сообщениями_ между сервисами, поэтому уместно звучит термин **«диаграмма сообщений»**. Каждое обращение к сервису в коде операции имеет взаимно однозначное отображение в стрелку-сообщение на диаграмме сообщений. Так обеспечивается наглядность операции.

{{<image src="images/auth_op.svg" title="Диаграмма сообщений для операции входа.">}}

Для обеспечения тестируемости операция инициализируется в фабрике операций. Объект, который будет запускать операцию на выполнение, имеет доступ только к фабрике операций, но не к самой операции. Объект запрашивает операцию у фабрики, и та возвращает интерфейс операции. Таким образом, `AppDelegate` остается тривиальным, а сама операция легко покрывается Unit-тестами. Все ее зависимости можно заменить mock-объектами и через обращения к ним отследить корректность цепочки сообщений.
