...
{{<alert context="success" icon="💡">}}
Время жизни операции ограничено, но в контексте iOS сильную ссылку на асинхронную операцию все равно нужно где-то хранить. Если этого не сделать, то при выходе из скоупа операция будет удалена, что не всегда допустимо. Удобно, если операция создает сильную ссылку сама на себя в инициализаторе, а при вызове _onFinish_ освобождает ее.
{{</alert>}}

«Неделимая последовательность» в определении операции накладывает два ограничения на ее код:
1. операция неделима, значит, не может включать в себя другие операции и не имеет к ним доступа;
1. сообщения в последовательности, значит, не должно быть логических развилок и циклов (условия и циклы нуждаются в состоянии, поэтому содержатся в сервисах).
...
{{<image src="images/auth_op.svg" title="Диаграмма сообщений для операции входа.">}}

Диаграмма сообщений может состоять из нескольких частей. Одна часть – постоянная. Это сообщения, сформированные непосредственно в методе `launch` и отвечающие за конфигурацию сервисов, за их взаимную настройку (подписки). Другая часть вариативная – это уже сообщения внутри этих предварительно настроенных сервисов. Такое разделение обусловлено тем, что сервисы содержат состояние и логические развилки, и для описания этой вариативной части может потребоваться несколько диаграмм.

{{<image src="images/auth_op_ext.svg" title="Диаграмма сообщений для операции входа: раздел AuthFlow.">}}
...
UI/UX — это один из источников [сложности]({{< relref "introduction#plasticity" >}}). Помимо этого, в UI сосредоточена большая часть платформенных различий между клиентскими приложениями. Инкапсулировать эту сложность позволяет **роутер**.
...
{{<alert context="success" icon="💡">}}
Каждая платформа обладает собственным инструментарием для работы с UI, поэтому роутер — это самая платформенно-зависимая сущность. Тем не менее, UI-логику тоже при желании можно сделать независимой от платформы. Применительно к iOS инстанцирование вью-контроллеров (и вообще всю работу с UIKit) можно перенести из роутера в фабрику вью-контроллеров. Таким образом, в роутере останется «чистая» UI-логика, которую можно будет покрыть Unit-тестами точно так же, как логику любого другого сервиса.
{{</alert>}}
...
Тестирование сценария осуществляются через остлеживание обращений к фабрике операций. Эти обращения должны быть в правильной последовательности, и такие тесты уже достаточно высокоуровневые, чтобы конкурировать с UI-тестами. Вместе с тем интеграционные тесты, проверяющие сценарий, остаются достаточно легкими и не хрупкими, сохраняя наглядность за счет использования диаграммы операций.
